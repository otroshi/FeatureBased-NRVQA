function [VIIDEO_corrs_features]=  compute_VIIDEO_features(vidfilename,height,width, ...
...
I=read_all_frames(vidfilename);
for frameitr = framevector   
    %[ydis1, ydis2]           = [(I(:,:,frameitr)),(I(:,:,frameitr+1))];%readframe(vidfilename,frameitr,height,width);
    ydis1=I(:,:,frameitr);
    ydis2=I(:,:,frameitr+1);
    %disp(frameitr)
...
VIIDEO_corrs_features       = computeVIIDEO_features_corrs(param);
...
%-------------------------------------------------------------------------
function corrs_features = computeVIIDEO_features_corrs(param)
...
for itr               = 1:round(gap/2):len
    ...
    for itr_param     = itr:min(itr+gap,len)
        ...
        [IDX    , ~]  = find(isinf(vec1)|isnan(vec1)|...
   ...
   if(~isempty(f1_cum))
    ...
    corrs_features   = [ corrs_features, C ];
   end
end
...
% 
% a=fseek(fid,(framenum-1)*width*height*1.5,'bof');
% disp(a)
